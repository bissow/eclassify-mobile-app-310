...
import 'package:eClassify/features/location/models/location_node.dart';
...
class LocationRepository {
  ...
  Future<Map<String, dynamic>> fetchLocation<T extends LocationNode>({
    ...
    try {
      final endPoint = switch (T) {
        Country => ApiEndpoints.getCountries,
        State => ApiEndpoints.getStates,
        City => ApiEndpoints.getCities,
        Area => ApiEndpoints.getAreas,
        _ => throw UnsupportedError('Unsupported Type $T'),
      };

      final idKey = switch (T) {
        Country => null,
        State => ApiParams.countryId,
        City => ApiParams.stateId,
        Area => ApiParams.cityId,
        _ => throw UnsupportedError('Unsupported Type $T'),
      };

      final response = await Api.get(
        url: endPoint,
        queryParameters: {?idKey: ?id, ApiParams.page: page},
      );
  ...
  Future<List<LeafLocation>> searchLocation({
    ...
    final usePaidApi = Constant.systemSettings.mapProvider.isPaidApi;
    try {
      final response = await Api.get(
        ...
        queryParameters: {
          ...
          ApiParams.lang: usePaidApi ? 'EN' : AppSession.currentLanguageCode,
        },
