...
import 'package:eClassify/features/jobs/repository/job_repository.dart';
...
class FetchJobApplicationCubit extends Cubit<FetchJobApplicationState> {
  ...
  void fetchApplications({
    ...
    try {
      ...
      DataOutput<JobApplication> result = await _jobRepository
          .fetchApplications(
            page: 1,
            itemId: itemId,
  ...
  Future<void> fetchMyMoreapplications({
    ...
    try {
      if (state is FetchJobApplicationSuccess) {
        ...
        emit(
          (state as FetchJobApplicationSuccess).copyWith(isLoadingMore: true),
        );
